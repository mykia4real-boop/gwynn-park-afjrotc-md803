(()=>{
  ...
  function addStyles(){
    ...
    s.textContent=`
      @media(min-width:1024px){
        ...
        .public-header>.sign-in{width:100%!important;min-height:44px!important;margin-top:7px!important;border-radius:11px!important;padding:9px 10px!important;white-space:nowrap!important;overflow:hidden!important;font-size:0!important;flex:0 0 auto!important}
        ...
        .site-rail-toggle{position:absolute;top:112px;right:-18px;width:36px;height:72px;border:0;border-radius:0 12px 12px 0;background:#ffd83d;color:#071528;box-shadow:4px 7px 18px rgba(15,23,42,.16);font-size:25px;font-weight:900;display:grid;place-items:center;cursor:pointer;z-index:70}
        ...
        .admin-side nav button{position:relative;width:100%!important;min-height:46px!important;padding:11px 12px!important;border-radius:11px!important;white-space:nowrap!important;overflow:hidden!important;font-size:0!important;color:#e8eef6!important}
        ...
        .admin-rail-toggle{position:absolute;top:112px;right:-18px;width:36px;height:72px;border:0;border-radius:0 12px 12px 0;background:#ffd83d;color:#071528;box-shadow:4px 7px 18px rgba(15,23,42,.16);font-size:25px;font-weight:900;display:grid;place-items:center;cursor:pointer;z-index:70}
  ...
  function decoratePublic(){
    ...
    header.querySelectorAll(".public-nav button[data-public]").forEach(btn=>{
      const k=publicKey(btn);btn.dataset.shellIcon=ICONS[k]||"•";
    });
    const admin=document.getElementById("adminPortalBtn");if(admin)admin.dataset.shellIcon="◆";
    const acct=document.getElementById("signInBtn");if(acct)acct.dataset.shellIcon="●";
    if(!header.querySelector(".site-rail-toggle")){
      ...
      toggle.onclick=()=>{
        ...
        toggle.textContent=open?"‹":"›";toggle.setAttribute("aria-label",open?"Collapse navigation panel":"Open navigation panel");
      };
      header.appendChild(toggle);
    }
    const saved=localStorage.getItem("afjrotcRailOpen")==="1";
    document.body.classList.toggle("site-rail-open",saved);
    const t=header.querySelector(".site-rail-toggle");if(t)t.textContent=saved?"‹":"›";
  }

  function decorateAdmin(){
    const side=document.querySelector(".admin-side");
    if(!side)return;
    side.querySelectorAll("nav button[data-admin]").forEach(btn=>{const k=btn.dataset.admin;btn.dataset.shellIcon=ICONS[k]||({announcements:"◉",calendar:"▣",uniform:"◇",groups:"◎",board:"□",resources:"▤",gallery:"▧",settings:"⚙"}[k]||"•")});
    if(!side.querySelector(".admin-rail-toggle")){
      ...
      toggle.onclick=()=>{
        ...
        toggle.textContent=open?"‹":"›";toggle.setAttribute("aria-label",open?"Collapse admin navigation panel":"Open admin navigation panel");
      };
    ...
    const t=side.querySelector(".admin-rail-toggle");if(t)t.textContent=saved?"‹":"›";
  }
  ...
  window.addEventListener("resize",()=>{if(!matchMedia(DESKTOP).matches){document.body.classList.remove("site-rail-open","admin-rail-open")}});
})();
